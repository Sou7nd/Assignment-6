...
class BinaryTree<T extends Comparable<T>> { 
  ...
  public T getMax() {
    ...
    return getMaxRecursively(root);
  }

  private T getMaxRecursively(Node<T> node) {
    if (node.right == null) {
        return node.element;
    }
    //right because larger numbers are to the right
  ...
  private boolean findRecursively(Node<T> node, T item){
    if (node == null) {
        return false;
    }
    int r = item.compareTo(node.element);
    if (r == 0) {
        return true;
    } else if (r < 0) {
        //Recursively check left
        return findRecursively(node.left, item);
    } else {
        //Recursively check right
        return findRecursively(node.right, item);
    }
  }
